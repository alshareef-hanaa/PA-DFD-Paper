...
# this function changes the format of DFD from dic to nested dic where key is the id of each DFD element (activators and flows)
...
def add_common_entities_pa_dfd(dfd_graph_typed,len_of_dfd_elements,limit_counter,request_counter,log_counter,DB_log_counter):
...
def generate_pa_dfd_graph(dfd_graph_typed):
        ...
        if data_flow_typed['style'] == 'endArrow=classic' and data_flow_typed['type'] == 'in':
            ...
            for key, value in dfd_graph_typed.items():
                if value['type'] == 'request' and value['value'].endswith(str(request_counter)):
                    ...
                    break
            dfd_graph_typed[len_of_dfd_elements] = {'id': len_of_dfd_elements, 'value': 'pol', 'style': 'endArrow=classic', 'source': data_flow_typed['source'],
    ...
    # *last version of transformation comp when* start by limiting first data and connect it with pol then log them and question if this d flow or not ?
    # transfer the 'comp' type of data flow
    for index, data_flow_typed in list(dfd_graph_typed.items()):
...
# functions
dfd_csv=initialize(dfd_xml_filename,dfd_csv_filename)
pa_dfd_csv=generate_pa_dfd_csv(dfd_csv_filename,pa_dfd_csv_filename)
pa_dfd_xml=generate_pa_dfd_xml(pa_dfd_csv_filename,pa_dfd_xml_filename)
